...
//        items.add(new ToDoItem("make party", "call to friends"));
    ...
    public ResponseEntity<Integer> saveTodo(ToDoItem todoItem) {

        //todoItem.setID((int) (Math.random() * Integer.MAX_VALUE));
        int returnID = todoService.save(todoItem);
    ...
    @PostMapping("/todo/delByID")
    public ResponseEntity delTodoItemByID(ObjectID objectID) {

        todoService.delete(objectID.getID());
